...
def main():
    ...
    # Build the tree
    tree_root = build_tree(X_train, y_train, max_depth=4)
    print("Decision Tree built done!")
    ...
    # Use the tree to get predictions
    y_test_pred = predict(tree_root, X_test)

    # Write predictions ("Jedi"/"Sith") to Tree.txt
    with open("Tree.txt", "w") as f_out:
    ...
    # f1 score
    if is_valition_needed:
        y_val_pred = predict(tree_root, X_val)
